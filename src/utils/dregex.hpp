...
type State struct {
    ...
    func
    newState(depth int) *
    State{S : = new (State) S.depth = depth S.success = treemap.NewWithIntComparator() return S}

    // insert sort a sort to keep it order
    func insertSorted(s[] int, e int)[] int {
i:
    = sort.Search(len(s), func(i int) bool{return (s)[i] <= e}) if i == len(s) { s = append(s, e) return s }
    if s
        [i] == e{return s} s = append(s, 0) copy(s [i + 1:], s [i:]) s[i] = e return s
}

// add emit
func(s *State) addEmit(keyword int) {
    if keyword
        != -100000 { s.emits = insertSorted(s.emits, keyword) }
}
...
func(s *State) isAcceptable() bool{return s.depth > 0 && len(s.emits) > 0}

    func(s *State) setFailure(failState *State, fail[] int){s.failure = failState fail[s.index] = failState.index}

    func(s *State) nextState(character int, ignoreRootState bool) *
    State {
    nextState, _ : = s.success.Get(character)

                         if (!ignoreRootState) &&
                     (nextState == nil) && (s.depth == 0) {
        nextState = s
    }
    if nextState
        == nil { return nil }
...
nextS:
    = s.nextState(character, true) if nextS == nil { nextS = newState(s.depth + 1) s.success.Put(character, nextS) }
    return nextS
}

// Builder is inner useed
type Builder struct {
    rootState *State trie *Trie
        /**
...
func(b *Builder) zipWeight(){
    msize : = b.size newBase : = make([] int, msize) copy(newBase[:b.size], b.trie.Base[:b.size]) b.trie.Base = newBase

    newCheck :
        = make([] int, msize, msize) copy(newCheck[:b.size], b.trie.Check[:b.size]) b.trie.Check = newCheck
}

func(b *Builder) constructFailureStates() {
    b.trie.Fail = make([] int, b.size + 1) b.trie.OutPut = make([][] int, b.size + 1) queue : = list.New()

                                                                                                    cpy
        : = func(ori[] int)[] int {
    dest:
        = make([] int, len(ori)) copy(dest, ori) return dest
    }

        for
            _, state : = range b.rootState.success.Values() {
            depthOneState:
                = state.(*State)depthOneState.setFailure(b.rootState, b.trie.Fail)
                      queue.PushBack(state) if len (depthOneState.emits) > 0 {
...
func(b *Builder) addAllKeyword(kvs StrLabelIter) int{
    ...
    t : = 0,
    -1,
    b.trie kvs(func(k StringIter, v[] int) bool {
         index++ lens : = 0 currentState : = b.rootState k(func(s * string, _ int) bool{
                                                  lens++ code : = b.trie.getcode(s) t.CodeMap[*s] = code currentState =
                                                      currentState.addState(code) return false
                                              }) currentState.addEmit(index) t.L =
                                               append(t.L, lens) if lens > t.MaxLen{t.MaxLen = lens} maxCode +=
                                           lens t.V = append(t.V, v)

                                               return false
     }) t.MaxLen++ return int(float32(maxCode + len(t.CodeMap)) / 1.5) +
        1
}
...
base2:
    = make([] int, newSize) check2 : = make([] int, newSize) used2
        : = make([] bool, newSize) if b.allocSize >
            0 {copy(base2[:int(b.allocSize)], b.trie.Base[:int(b.allocSize)])
                   copy(check2[:int(b.allocSize)], b.trie.Check[:int(b.allocSize)])
                       copy(used2[:int(b.allocSize)], b.used[:int(b.allocSize)])} b.trie.Base = base2 b.trie.Check =
                check2 b.used = used2 b.allocSize = newSize
}
...
tCurrent:
    = queue.Remove(queue.Front()).(Pair)value,
        siblings : = tCurrent.K.(int),
                 tCurrent.V.([] Pair)

                     begin,
                 nonZeroNum : = 0, 0 first : = true pos : = b.nextCheckPos - 1 if pos < siblings[0].K.(int) {
	...
	t := b.trie
	for {
            pos++ if b.allocSize <= pos { b.resize(pos + 1) }
            if t
                .Check[pos] != 0 { nonZeroNum++ continue }
            else if first {
                b.nextCheckPos = pos first = false
            }

            begin = pos - siblings[0].K.(int)if b.allocSize <= (begin + siblings[len(siblings) - 1].K.(int)) {
		...
		for i := 0;
            ...
            i++ {
                if t
                    .Check[begin + siblings[i].K.(int)] != 0 { allIszero = false break }
            }
            if allIszero {
                break
            }
    ...
    if float32 (nonZeroNum)
        / float32(pos - b.nextCheckPos + 1) >= 0.95 {b.nextCheckPos = pos} b.used[begin] =
            true
        ...
        i:
            = 0;
        i < len(siblings);
        i++ { t.Check[begin + siblings[i].K.(int)] = begin }
        for
        i:
            = 0;
        i < len(siblings);
        i++ {
        kv:
            = siblings[i] newSiblings : = fetch(kv.V.(*State)) if len (newSiblings) < 1 {
                t.Base[begin + kv.K.(int)] = -(kv.V.(*State).getMaxValueIDgo() + 1)
            }
            else {queue.PushBack(Pair{begin + kv.K.(int), newSiblings})} kv.V.(*State).index = begin + kv.K.(int)
        }
        if value
            >= 0 { t.Base[value] = begin }
}
...
maxCode:
    = b.addAllKeyword(kvs)
      ...
      b.resize(maxCode + 10) b.trie.Base[0] = 1 siblings : = fetch(b.rootState) if len (siblings) > 0 {
    queue:= list.New()
    ...
    b.constructFailureStates() b.rootState = nil b.zipWeight()
}

// CompileTrie the data
func CompileTrie(kvs StrLabelIter) * Trie {
    var builder Builder builder.rootState = newState(0) builder.trie = newTrie() builder.build(kvs)
                                                                       // clear mem
                                                                       result
        : = builder.trie builder.trie = nil return result
}
