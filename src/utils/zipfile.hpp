...
#ifndef SRC_UTILS_ZIPFILE_HPP_
#define SRC_UTILS_ZIPFILE_HPP_
...
class ZipFileReader {
   ...
   public:
    ...
    explicit ZipFileReader(const std::string& filename);
...
inline void Swap_Endianity(T& x) {
    ...
    if (sizeof(T) > 1) {
        T old = x;
        for (uint32_t k = 1; k <= sizeof(T); k++) ((char*)&x)[k - 1] = ((char*)&old)[sizeof(T) - k];
    }
}
...
struct GZipFileHeader {
    uint8_t magic0, magic1;  // magic should be 0x8b,0x1f
    uint8_t cm;              // compression method 0x8 is gzip
    uint8_t flags;           // flags
    uint32_t modtime;        // 4 byte modification time
    uint8_t flags2;          // secondary flags
    uint8_t os;              // operating system 0xff for unknown
    ...
    bool Read(std::istream& istream) {
        ...
        for (int i = 0; i < stringsToRead; i++) do {
                Read_Primitive(istream, dummyByte);
            } while (dummyByte != 0 && istream);
        if (flags & 1) Read_Primitive(istream, crc16);
        if (!istream) {
            std::cerr << "gzip: got to end of file after only reading gzip header" << std::endl;
            return false;
        }
        return true;
    }
...
struct ZipFileHeader {
    ...
    explicit ZipFileHeader(const std::string& filename_input)
        : version(20),
          flags(0),
          compression_type(8),
          stamp_date(0),
    ...
    bool Read(std::istream& istream, const bool global) {
        ...
        if (global) {
            ...
            uint16_t disk_number_start, int_file_attrib;
            uint32_t ext_file_attrib;
    ...
    void Write(std::ostream& ostream, const bool global) const {
        if (global) {
            Write_Primitive(ostream, (uint32_t)0x02014b50);  // header sig
            Write_Primitive(ostream, (uint16_t)00);
        } else {
            Write_Primitive(ostream, (uint32_t)0x04034b50);
        }
        ...
        Write_Primitive(ostream, (uint16_t)filename.length());
        Write_Primitive(ostream, (uint16_t)0);  // extra lengthx
        if (global) {
            Write_Primitive(ostream, (uint16_t)0);  // filecomment
            Write_Primitive(ostream, (uint16_t)0);  // disk# start
            Write_Primitive(ostream, (uint16_t)0);  // internal file
            Write_Primitive(ostream, (uint32_t)0);  // ext final
            Write_Primitive(ostream, (uint32_t)header_offset);
        }  // rel offset
        for (uint32_t i = 0; i < filename.length(); i++) Write_Primitive(ostream, filename.c_str()[i]);
    }
};

// class ZipStreambufDecompress
static const uint32_t _MAX_BUFFER_SIZE = 1024 * 1024;

class ZipStreambufDecompress : public std::streambuf {
    std::istream& istream;

    z_stream strm;
    uint8_t in[_MAX_BUFFER_SIZE]  = {0};
    uint8_t out[_MAX_BUFFER_SIZE] = {0};
    ...
    static const uint16_t DEFLATE      = 8;
    static const uint16_t UNCOMPRESSED = 0;
   ...
   public:
    ...
    int process() {
        if (!valid) return -1;
        if (compressed_data) {
            strm.avail_out = _MAX_BUFFER_SIZE - 4;
            strm.next_out  = (Bytef*)(out + 4);
            while (strm.avail_out != 0) {
                if (strm.avail_in == 0) {  // buffer empty, read some more from file
                    uint32_t rsize = (uint32_t)_MAX_BUFFER_SIZE;
                    if (part_of_zip_file) {
                        rsize = std::min((uint32_t)_MAX_BUFFER_SIZE, header.compressed_size - total_read);
                    }
                    istream.read((char*)in, rsize);
            ...
            int unzip_count = _MAX_BUFFER_SIZE - strm.avail_out - 4;
            total_uncompressed += unzip_count;
            return unzip_count;
        } else {  // uncompressed, so just read
            istream.read((char*)(out + 4), std::min(_MAX_BUFFER_SIZE - 4, header.uncompressed_size - total_read));
...
class ZipStreambufCompress : public std::streambuf {
   ...
   public:
    ZipStreambufCompress(ZipFileHeader* header, std::ostream& stream) : ostream(stream), header(header), valid(true) {
        ...
        setg(0, 0, 0);
        setp((char*)in, (char*)(in + _MAX_BUFFER_SIZE - 4));  // we want to be 4 aligned
        // Write appropriate header
        if (header) {
            header->header_offset = static_cast<uint32_t>(stream.tellp());
            header->Write(ostream, false);
        } else {
            header_offset = static_cast<uint32_t>(stream.tellp());
   ...
   protected:
    int process(bool flush) {
        ...
        while (strm.avail_in != 0 || flush) {
            strm.avail_out = _MAX_BUFFER_SIZE;
        ...
        setp(pbase(), pbase() + _MAX_BUFFER_SIZE - 4);
        return 1;
    }
...
ZipFileWriter::~ZipFileWriter() {
    ...
    Write_Primitive(ostream, (uint32_t)0x06054b50);                      // end of central
    Write_Primitive(ostream, (uint16_t)0);                               // this disk number
    Write_Primitive(ostream, (uint16_t)0);                               // this disk number
    Write_Primitive(ostream, (uint16_t)files.size());                    // one entry in center in this disk
    Write_Primitive(ostream, (uint16_t)files.size());                    // one entry in center
    Write_Primitive(ostream, (uint32_t)(central_end - final_position));  // size of header
    Write_Primitive(ostream, (uint32_t)final_position);                  // offset to header
    Write_Primitive(ostream, (uint16_t)0);                               // zip comment
}
...
ZipFileReader::~ZipFileReader() {
    ...
    for (; i != filename_to_header.end(); ++i) delete i->second;
}
...
bool ZipFileReader::Find_And_Read_Central_Header() {
    ...
    uint32_t max_comment_size         = 0xffff;  // max size of header
    uint32_t read_size_before_comment = 22;
    ...
    uint32_t word;
    uint16_t disk_number1, disk_number2, num_files, num_files_this_disk;
    ...
    uint32_t size_of_header, header_offset;
...
void ZipFileReader::Get_File_List(std::vector<std::string>& filenames) const {
    ...
    for (; i != filename_to_header.end(); ++i) filenames.push_back(i->first);
}
