...
template <typename value_type, const int NO_VALUE = NaN<value_type>::N1, const int NO_PATH = NaN<value_type>::N2,
          const bool ORDERED = true, const int MAX_TRIAL = 1, const size_t NUM_TRACKING_NODES = 0>
class da {
   public:
    ...
    int save(std::ostream& os) const {
        ...
        os.write(reinterpret_cast<const char*>(_array), std::streamsize(_size * sizeof(node)));
        os.flush();
        return 0;
    }
