...
uint8_t utf8_previous(utf8_iter* iter);  // returns 1 if there is a character in the back
                                         // position. If there is not, return 0.

const char* utf8_getchar(utf8_iter* iter);  // return current character in UFT8 - no same that
...
uint8_t utf8_charsize(const char* character);  // calculate the number of bytes a UTF8 character
                                               // occupies in a string.
uint8_t unicode_charsize(uint32_t codepoint);  // calculates the number of bytes occupied by a
