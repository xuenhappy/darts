...
void save_sparse(Eigen3Hdf5_H5CommonFG& h5group, const std::string& name, const SparseMatrixType& mat,
    ...
    for (int k = 0; k < mat.outerSize(); ++k) {
        for (typename SparseMatrixType::InnerIterator it(mat, k); it; ++it) {
            if (it.value() != Scalar(0)) data.push_back(MyTriplet<Scalar>(it.row(), it.col(), it.value()));
        }
    }
