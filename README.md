...
```mermaid
flowchart LR
    INPUT["UTF-8 输入文本"] --> NORMALIZE["归一化与字符类型识别"]
    NORMALIZE --> ATOMS["AtomList 原子序列<br/>统一位置索引"]

    subgraph RECOGNIZERS["候选识别层 Recognizers"]
        BASE["单 Atom 基础候选"]
        DICT["双数组 Trie 词典识别<br/>通用词典 / LAC 词典 / POI 词典"]
        RULE["规则状态机<br/>日期时间 / 数量单位 / 地址"]
        BINARY["neural span 识别器<br/>2 至 5 Atom 成词概率"]
        SYNTAX["lac-nural 多分类识别器<br/>NOT_WORD + POS 类型"]
    end

    ATOMS --> BASE
    ATOMS --> DICT
    ATOMS --> RULE
    ATOMS --> CODEC["WordPiece + 位置编码"]
    CODEC --> SHARED["共享 Transformer 词表示网络"]
    SHARED --> BINARY
    SHARED --> SYNTAX

    BASE --> GRAPH["SegPath 候选词 DAG"]
    DICT --> GRAPH
    RULE --> GRAPH
    BINARY -->|"neural 模式"| GRAPH
    SYNTAX -->|"lac-nural 模式"| GRAPH

    GRAPH --> ENCODE["候选 span + 词类型编码"]
    CODEC --> INDICATOR["ONNX Indicator<br/>候选词嵌入"]
    ENCODE --> INDICATOR
    INDICATOR --> QUANTIZER["ONNX Quantizer<br/>相邻词关联概率负对数"]
    GRAPH --> QUANTIZER

    GRAPH --> DECIDER{"可配置 Decider"}
    QUANTIZER -->|"neural / lac-nural"| DECIDER
    STAT["MinCover / Bigram / HybridStat"] -->|"非神经模式"| DECIDER
    DECIDER --> PATH["Dijkstra / 温度采样<br/>完整路径选择"]
    PATH --> WORDS["WordList<br/>词面 + 原文偏移 + 标签 / POS"]
    WORDS --> API["C API / Cython / Python Tokenizer"]

    TRAIN["空白分词语料 + POS 语料"] -.-> JOINT["PyTorch 联合训练<br/>识别损失 + GraphLossSparse"]
    JOINT -.-> SHARED
    JOINT -.-> BINARY
    JOINT -.-> SYNTAX
    JOINT -.-> QUANTIZER
    JOINT -.-> EXPORT["分别导出 ONNX<br/>Recognizer / Indicator / Quantizer"]
    EXPORT -.-> BINARY
    EXPORT -.-> SYNTAX
    EXPORT -.-> INDICATOR
    EXPORT -.-> QUANTIZER
...
运行时所有候选位置、训练标签和 `best_path` 都使用同一套 Atom 半开区间索引。
`neural` 与 `lac-nural` 共享词表示架构，但分别训练并导出识别器、Indicator 和
Quantizer；量化器输出相邻候选关联概率的负对数，而不是无约束 score。
...
项目根目录的 [`demo.py`](demo.py) 提供了一个可直接运行的复杂文本演示。脚本复用
同一批分词器实例，覆盖最佳路径、重叠候选、歧义路径采样、拼音消歧、地址 POI
解析、LAC 词性标注、神经网络模式和简单吞吐测试：
...
- “南京市长江大桥”“研究生命起源”“乒乓球拍卖完了”等多路径歧义句。
- 中文、英文、URL、邮箱、版本号和标点混排。
- `2026年7月20日14:30`、`2.5GB`、金额、百分比等日期时间和数量单位。
- “重庆、银行、行长、音乐”等多音字的句子级拼音消歧。
- 省、市、区、街道、道路、门牌、楼层和 POI 的结构化地址解析。
- `hybrid`、`neural`、`lac` 与 `lac-nural` 模式的结果对照。

可以通过参数控制主演示模式、温度采样和测试规模：
...
| 参数 | 默认值 | 说明 |
| --- | ---: | --- |
| `--config` | `data/conf.json` | 运行时 JSON 配置文件 |
| `--mode` | `hybrid` | 复杂文本最佳路径和吞吐测试使用的模式 |
| `--temperature` | `0.8` | 歧义路径采样温度；越高通常多样性越大 |
| `--samples` | `8` | 每个歧义句的采样次数 |
| `--candidate-limit` | `60` | 最多显示的重叠候选词数量 |
| `--rounds` | `5` | 吞吐测试轮数 |
| `--no-benchmark` | 关闭 | 跳过吞吐测试，便于只观察切分结果 |

`neural` 或 `lac-nural` 所需 ONNX 文件不可用时，脚本会打印跳过原因并继续运行其他
章节。输出偏移均为 Python 字符串的半开区间 `[start, end)`，可直接使用
`text[start:end]` 还原原始词面。
...
识别器的局部候选指标采用两套 gold，避免把合法歧义词误报为错误候选：

- precision：预测 span 只要命中训练词典、规则词或当前句标注词，就视为合法词。
- recall：仍以当前句语料标注 span 为分母，衡量标注词位置覆盖率。
- 多分类 POS accuracy：只在语料标注 span 上比较类别；另行报告实际被阈值选中的
  标注词 POS accuracy，词边界评价与标签评价不混合。

二分类联合训练先统计完整训练集的正负 span 比例，并通过
`--max-positive-weight` 固定正类 BCE 权重，避免权重随 batch 组成波动。设置为 `1`
等价于未加权 BCE。当前消融中，未加权模型的局部候选 F1 有提升，但独立 test 的最终
路径 F1 仅从 0.8799 变化到 0.8803，且概率校准未改善，因此发布模型仍保留原版本。
后续优化应优先隔离 GraphLossSparse 对共享 encoder 概率尺度的梯度影响，并对词典
命中但不在当前唯一标注路径上的 span 使用降权或 PU 监督，而不是把它们作为硬负例。
